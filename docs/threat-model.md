...
| 1.5 | Connection flood saturates accept queue / fds | D | 🟢 6.5.1: max concurrent connections (`CMCP_HTTP_MAX_CONNECTIONS`, default 64) + token-bucket accept-rate (`CMCP_HTTP_ACCEPT_RATE`, default 100/s, burst 200). Over-capacity → 503 + `Retry-After`. |
| 1.6 | DNS-rebinding attack on a localhost-bound server | S | 🟢 `Origin` allow-list (`CMCP_HTTP_ALLOWED_ORIGINS`, MCP 2025-11-25 Minor 3); mismatched `Origin` → 403. |
